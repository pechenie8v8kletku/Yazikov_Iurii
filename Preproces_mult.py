...
detector = None
# в этой части кода реализован поиск bbox с мультипроцом через пулы, модель реализованная через CNN у меня не запустилась скорее всего проблема в версиях библиотек
# wheel Для cuda не был найден а когда все же удалось настроить dlib конфликтовал с cuDNN так что был взят вариант без сверточной нейросети,также был использован падинг в 15 процентов,вокруг
# выделенного bbox т.к. на этапе трейна это может дать прирост для CNN возмонжо стоит взять его побольше, но задача больше про подбор архитектуры так что такой ресерч остался для улучения.
# также плохо размеченные изображения IOU<0.3 у ббоксов убирались из трейна но не теста.
# было бы интересно попробовать через модель поискать лица но не запустилось
# Также чтобы этот код запустился должна быть data и библиотеки из requirments на xeon e5 2670v3 обработка заняла 5 мин
